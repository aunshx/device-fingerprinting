...
X = [item["operationOutput"] for item in processed_data]
y = [item["platform"] for item in processed_data]
...
# Compute DTW distances with progress tracking
def dtw_distance_matrix(X, desc="Computing DTW distances"):
    size = len(X)
    matrix = np.zeros((size, size))
    for i in tqdm(range(size), desc=desc, position=0):
        for j in range(i, size):
            distance = dtw.distance(X[i], X[j])
            matrix[i, j] = distance
            matrix[j, i] = distance
    return matrix
...
for i in tqdm(range(len(X_test)), desc="Test set DTW", position=0):
    for j in range(len(X_train)):
        test_distance_matrix[i, j] = dtw.distance(X_test[i], X_train[j])
